##ARQUITETURA: Algoritmo de Booth com interface gráfica para o usuário (versão do trabalho)
...
import sys      #Módulo com funções relacioandas ao sistema
...
class multiplicacao:
    ...
    def __init__(self,multiplicando,multiplicador):
        ...
        #print('INÍCIO.')
        regs.append(['0','0','0','0'])
        outros.append(['Operação: '+str(multiplicando)+' x '+str(multiplicador)])
        imgs.append('imagens/1.png')
        if(manual==''):
                manual= input()
        
        #Vamos converter de decimal para binário
        multiplicando=conversoes.decimal_binario(multiplicando)
        multiplicador=conversoes.decimal_binario(multiplicador)
        ...
        while (True):               #Iteração
            ...
            if(contador==0):
                ...
                decimal=conversoes.binario_decimal(resultado)
                regs.append([pc.A,pc.Q,pc.C,pc.M])
                outros.append([ 'Resultado: ',resultado,str(decimal)])
                imgs.append('imagens/7.png')
                break
            
        return None

#Funções para realizar a conversoes entre decimal e binario
class conversoes:
    #Função para converter o número de decimal para binário
    def decimal_binario(decimal):
        #decimal    - Número em decimal
        
        decimal=int(decimal)                #Vamos garantir que o número seja um inteiro

        if(decimal < -16 or decimal > 15):  #Checar se e um número válido para nosso tamanho de bits:
            print('Número inválido')
            sys.exit()                      #Paramos a execução do programa
        else:
            modulo=abs(decimal)             #Pegamos o módulo do número
            valores=[8,4,2,1]               #'Valor' de cada posição
            binario='0'                     #Onde vamos guardar nosso valor binário
            for valor in valores:           #Vamo 'distribuir' o valor total entre os binários
                if (valor <= modulo):       #Se ele 'cabe' na posição
                    binario=binario+'1'     #Salvamos
                    modulo=modulo-valor     #E diminuimos do valor que resta distribuir
                else:
                    binario=binario+'0'     #Se não, vamos pra próxima posição
            if(decimal<0):                  #Se o valor era negativo, pegamos a negação
               binario=subtracao.negacao(binario)
            return binario                  #Retornamos o valor binario

    #Função para converter um número de binario para decimal
    def binario_decimal(binario):
        #binario    - Número em binario
        valores=[256,128,64,32,16,8,4,2,1]              #'Valor' de cada posição
        dec=0                                           #Onde vamos guardar o valor decimal
        sin=1                                           #Vamos usar pra guardar o sinal
        if (binario[0]=='1'):                           #Se o binário é negativo
            binario=subtracao.negacao(binario)          #Fazemos a negação
            sin=-1                                      #Guardamos o sinal
        for pos in range(1,10):                         #Vamos percorrar o binário a partir do segundo bit mais significativo
            if(binario[pos]=='1'):                      #Se for valor 1
                dec=dec+valores[pos-1]                  #Somamos a posição correspondente
        dec=dec*sin                                     #Recuperamos o sinal
        return(dec)                                     #Retornamos o valor em decimal
...
while True:
    ...
    for qt in range(len(outros[c])):
        texto=fonte.render(outros[c][qt],1,PRETO)
        textoret=texto.get_rect()
        textoret.topleft=(150,450+30*qt)
        screen.blit(texto,textoret)
